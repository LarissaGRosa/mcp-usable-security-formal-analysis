...
### 6.8 Realization status (2026-06-23)

§6 is **implemented** in `tamarin_model/` as profiles $\mathcal{E}_0$ (Experiment 01), plus $\mathcal{E}_2 = \langle C_0{+}\text{UI}, \{\text{Attentive},\text{Habituated}\}, \{\sigma_8\}\rangle$ (Experiment 02) and $\mathcal{E}_3 = \langle C_0, \{\text{Attentive},\text{Careless}\}, \{\sigma_2\}\rangle$ (Experiment 03), atop the Experiment-00 baseline; a merged worst-case profile $\mathcal{E}_4 = \langle C_0{+}\text{UI}, \{\text{Attentive},\text{Busy},\text{Careless},\text{Habituated}\}, \{\sigma_1,\sigma_2,\sigma_8\}\rangle$ (Experiment 04) exercising stressor *interaction*; and a recovery profile $\mathcal{E}_5$ (Experiment 05) where a good-usability UI warning returns a Habituated user to Attentive. All six entry-point theories prove under Tamarin 1.12 (28 lemmas, each ≤2 s) — $\mathcal{E}_4$ confirming §6.5's claim that orthogonal experiments compose without the per-stressor safety properties degrading. Confirmations and refinements to §6.7:
...
- **Persistent current mask across actions (refines §6.2).** The mask is not re-derived per action: each stressor emits `SetMask(p,m)` at onset, the recovery warning emits `SetMask(p,'Attentive')`, and the gates in `core/transitions/mask_state.spthy` make every f_H respond as the *current* mask, which **persists across action types** (CALC_SK → APPROVE_REQ) until a recovery. This fixes the memoryless §3.2 reading — a user driven Busy on the KDF stays degraded at the approval prompt (proven: `LM_mask_persists_across_actions`). The f_M edge is now the trigger's `SetMask`; the per-edge gate files are gone.
- **Recovery = a monotone positive event, derived by ordering.** Reverting to Attentive (§7 of the plan) is *without* un-doing the stressor: the UI-warning's `SetMask(p,'Attentive')`, post-dating a degrade, re-enables the Attentive response and disables the degraded one — until a later stressor re-degrades.
- **"Active until recovery", not strict latest-wins (a tractability choice).** The current-mask gate reads a degrade as active iff some `SetMask(p,m)` has no later recovery `SetMask(p,'Attentive')` — scanning only for a recovery between, NOT for *any* `SetMask` between. The stricter "latest mask of any kind wins" is correct but its all-traces search **blows up once ≥3 stressors interleave** (it timed out at >290 s on the merged experiment); the "active until recovery" reading forbids the spurious Busy→Attentive revert and keeps recovery, while letting a new stressor *add* an active mask rather than replace the old — a safe over-approximation, ≤2 s.
- **Bound state-establishing events for termination.** Events that *latch* a state and are emitted by rules reading only persistent facts (`Load`, `Distract`, `Habituate`; and `SetMask`) must be **capped to once per party** (`OneStressPerParty`/`OneDistractPerParty`/`OnceHabituate`, and `OneSetMaskPerMask`). Otherwise such a rule re-fires unboundedly and any gate doing *interval* reasoning over those events fails to terminate. The cap is semantically free (re-latching a state you are already in changes nothing) and keeps proofs ≤2 s.
