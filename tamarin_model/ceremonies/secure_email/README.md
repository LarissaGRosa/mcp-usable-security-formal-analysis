...
`masks/attentive_share.spthy`, `masks/careless_share.spthy`, `masks/busy_share.spthy`,
`stressors/time_pressure_share.spthy`.
...
step deposits the **objective operation** it performs, and a `core/` *inferred* detector reads its
tag and infers the stressor — the HCI judgment lives in the detector, not a designer flag
